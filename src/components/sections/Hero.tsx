...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* ── DESKTOP UNIQUEMENT : panneau rouge diagonal + carousel + ligne or + badge ── */}
      ...
      <svg
        ...
        aria-hidden="true"
      >
        <line x1="63.04%" y1="0%" x2="58%" y2="100%" stroke="#E0A238" strokeWidth="1.5" opacity="0.65" />
      </svg>

      {/* Badge desktop — flottant en bas à droite */}
      <motion.div
      ...
      {/* ── CONTENU PRINCIPAL ── */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex-1 flex flex-col justify-start md:justify-center">

        {/* Photo pleine largeur — MOBILE UNIQUEMENT, en haut, sans filtre */}
        <div className="md:hidden -mx-4 sm:-mx-6 relative mb-0" style={{ height: '260px' }}>
          <AnimatePresence mode="sync">
            <motion.img
            ...
            {heroImages.map((img, i) => (
              <button
                ...
                style={{
                  ...
                  height: '6px',
                  borderRadius: '3px',
                  background: i === current ? '#E0A238' : 'rgba(255,255,255,0.6)',
                  border: 'none',
                  cursor: 'pointer',
                  padding: 0,
          ...
          {/* Badge mobile — incrusté sur la photo, bas droite */}
          <motion.div
            initial={{ opacity: 0, scale: 0.5, rotate: -15 }}
            animate={{ opacity: 1, scale: 1, rotate: 0 }}
            transition={{ duration: 0.6, delay: 1.0, type: 'spring', bounce: 0.5 }}
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                transition={{ duration: 2.2, repeat: Infinity, ease: 'easeOut', delay: 1.1 }}
              />
              <div className="relative w-20 h-20 rounded-full bg-white shadow-xl border-2 border-gold flex flex-col items-center justify-center select-none">
                <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-red-lcd" />
                <span className="font-heading text-red-lcd text-[6px] font-bold tracking-widest uppercase">Places</span>
                <span className="font-heading text-navy font-extrabold text-base leading-none">limitées</span>
                <div className="w-5 h-px bg-gold my-1" />
                <span className="font-heading text-gold text-[7px] font-bold tracking-wider uppercase">Oct. 2026</span>
              </div>
            </motion.div>
          </motion.div>
        </div>
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="pt-6 pb-8 md:pt-0 md:pb-0 md:w-[54%] lg:w-[50%]"
        >
          ...
          <motion.p
            variants={itemVariants}
            className="font-body text-text-soft/75 text-base md:text-lg leading-relaxed mb-8 max-w-lg"
          >
            {t('hero.soustitre')}
          </motion.p>
